...
export function createMetaArtWorkItem({ id, collection = null, title = '', artist = '', date = '', place = '',
  url = null,
  type = null, dataKey = null }) {

  // Sans dataKey : simple artwork, sans vue embarquée
  if (!dataKey) {
    return createArtWorkItem({ id, collection, title, artist, date, place, url, type, transformations: null })
  }
  ...
  return createArtWorkItem({ id, collection, title, artist, date, place, url, type, transformations: transformations })
}
