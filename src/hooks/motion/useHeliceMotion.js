...
function helixPoint(radius, cVert, u) {
  return new THREE.Vector3(radius * Math.cos(u), cVert * u, radius * Math.sin(u))
}

export function useHeliceMotion(radius, cVert, speed, direction, phase = 0) {
  const ref = useRef()

  useFrame(({ clock }) => {
    if (!ref.current) return
    const u = clock.getElapsedTime() * speed * direction + phase

    const point = helixPoint(radius, cVert, u)
    const next = helixPoint(radius, cVert, u + 0.001)
