...
//data = datafileKey, exp. : peopleKey / people.json, astroKey / astro.json, karnakKey.karnak.json
...
export function useArtworkLoader(data) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const data = await dataItem.loader()
